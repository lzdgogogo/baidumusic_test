...
class taihe_phonenumber_login_page(base_utils):
        ...
        def go_to_account_login_page(self):
                ...
                if not self.find_element_and_action(By.XPATH,self.go_to_account_login_button_xpath,self.action.click,'去账号密码登录页') == 0:
        ...
        def go_to_baidu_login_page(self):
                """功能：
                        点击百度账号登录按钮，去百度登录页"""
                if not self.find_element_and_action(By.XPATH,self.go_to_baidu_login_button_xpath,self.action.click,'百度账号登录') == 0:
                    log_utils.F_ERROR('点击失败')
                    return 1
        ...
        def go_to_other_login_page(self):
                """功能：
                        点击其他方式登录按钮"""
                if not self.find_element_and_action(By.XPATH,self.go_to_other_login_button_xpath,self.action.click,'其他方式登录') == 0:
                    log_utils.F_ERROR('点击失败')
                    return 1
