...
class dialog_page(base_utils):
        ...
        def click_yes_button(self):
                """点击确定（此按钮一直在右边）"""
                if not self.find_element_and_action(By.ID,self.yes_button_id,self.action.click,'确定按钮') == 0:
                    log_utils.F_FAIL('点击确定按钮失败')
                    return 1

        def click_cancel_button(self):
                """点击取消（此按钮在左边）"""
                if not self.find_element_and_action(By.ID,self.cancel_button_id,self.action.click,'取消按钮') == 0:
